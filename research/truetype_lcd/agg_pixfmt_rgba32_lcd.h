...
namespace agg
{
                               ...
                               const int8u* covers)
        {
            ...
            for (int x = x_min; x <= x_max; x++)
            {
                for (int i = 0; i < 3; i++) {
                    ...
                    unsigned src_col = p[i];
                    p[i] = (int8u)((((dst_col - src_col) * alpha) + (src_col << 16)) >> 16);
                }
                p[3] = 0xff;
                p += 4;
            }
    ...
    public:
        ...
        //--------------------------------------------------------------------
        pixfmt_rgba32_lcd_gamma(rendering_buffer& rb, const lcd_distribution_lut& lut, const Gamma& gamma)
                               ...
                               const int8u* covers)
        {
            ...
            for (int x = x_min; x <= x_max; x++)
            {
                for (int i = 0; i < 3; i++) {
                    ...
                    unsigned src_col = m_gamma.dir(p[i]);
                    p[i] = m_gamma.inv((((dst_col - src_col) * alpha) + (src_col << 16)) >> 16);
                }
                p[3] = 0xff;
                p += 4;
            }
